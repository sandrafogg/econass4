...
##Marginal Carbon Abatement Costs and Demand 
...
####Graphical Representation of Marginal Abatement Costs 
...
###Demand of Emissions
At any quanity, x, the marginal cost of abatement can be obtained using the above functions and substracting "x" from the current emissions scenario. The demand curves for each sector were determined through sequential marginal willingness to pay estimations.
```{r, message = FALSE, echo = FALSE, warning=FALSE, include = FALSE}
...
####Policy Option 1: A Cap on Carbon
...
2. The Total Cost of a Carbon Cap for Each Sector      
...
3. The Tax Revenue Generated from a Carbon Cap  

There is no tax revenue generated with a carbon cap



####Policy Option 2: A Tax on Carbon
...
1. The Total Cost of a Carbon Tax
...
2. The Total Cost of a Carbon Tax for Each Sector
...
```
####Policy Option 3: A Cap and Trade Program

1. The Total Cost of a Carbon Cap and Trade Program
...
From the previous question, if country Y enters the carbon market, Country X will be emitting `r round(q_xy_d, digits=2)` more, so the amount of air pollution will be $ `r round(total_emission_x, digits=2)`

```{r include=FALSE}
total_emission_y <- 300-q_xy_d
```

Since the Country Y will be abating `r round(q_xy_d, digits=2)`  more, they will be emitting `r round(total_emission_y, digits=2)` tons of carbon, and that will result in `r round(total_emission_y, digits=2)` tons of air pollution.


####c. Advice on International Trade of Carbon Emission Credits  

Allowing international trade of carbon emissions is economically efficient but the local impacts of carbon emissions is not captured by this model. If Country Y enters to the market, Country X will be emitting more carbon which will cause a lot of local air pollution.
